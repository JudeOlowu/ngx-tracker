...
pub fn export_html(stocks: &[Stock]) -> Result<String> {
    let payload  = serde_json::to_string(&build_payload(stocks))?;
    let injected = format!("var INJECTED = {}; /* <<NGX_DATA>> */", payload);
    let output   = HTML_TEMPLATE.replace(MARKER, &injected);
    fs::write("ngx_live.html", &output)?;
    Ok("ngx_live.html".to_string())
}

pub fn build_html_string(stocks: &[Stock]) -> String {
    let payload  = serde_json::to_string(&build_payload(stocks))
        .unwrap_or_else(|_| "{}".to_string());
    let injected = format!("var INJECTED = {}; /* <<NGX_DATA>> */", payload);
    HTML_TEMPLATE.replace(MARKER, &injected)
}
...
fn build_payload(stocks: &[Stock]) -> Value {
    json!({
